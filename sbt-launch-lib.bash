...
checkJava() {
  ...
  # Now check to see if it's a good enough version, this is not evaluating correctly on windows 10
  local good_enough="$(echo $java_version ">=" $required_version | bc -l)"
...
run() {
  ...
  if [[ "$JAVA_VERSION" == "1.8" ]]; then echo "JAVA_VERSION env var is set to $JAVA_VERSION: skip java check"
  else checkJava "1.6"
  fi
